<?php

namespace Tests\BBC\ProgrammesPagesService\Service\ContributorsService;

use BBC\ProgrammesPagesService\Domain\Entity\Contributor;
use BBC\ProgrammesPagesService\Domain\Entity\Service;
use BBC\ProgrammesPagesService\Domain\ValueObject\Pid;
use BBC\ProgrammesPagesService\Domain\ValueObject\Sid;

class CountPlaysForTimeByPidTest extends AbstractContributorsServiceTest
{
    public function testCounts()
    {
        $from = new \DateTimeImmutable('2016-06-01');
        $to = new \DateTimeImmutable('2016-06-07');

        $mbid1 = '7746d775-9550-4360-b8d5-c37bd448ce01';
        $mbid2 = '9c9f1380-2516-4fc9-a3e6-f9f61941d090';

        $pid1 = 'cntrbtr1';
        $pid2 = 'cntrbtr2';

        $contributor1 = new Contributor(
            1,
            new Pid($pid1),
            'person',
            'Number 1',
            $mbid1
        );
        $contributor2 = new Contributor(
            2,
            new Pid($pid2),
            'person',
            'Number 1',
            $mbid2
        );

        $dbData = [
            [
                0 => [
                    'pid' => $pid1,
                    'musicBrainzId' => $mbid1,
                ],
                'contributionPlays' => 10,
            ],
            [
                0 => [
                    'pid' => $pid2,
                    'musicBrainzId' => $mbid2,
                ],
                'contributionPlays' => 5,
            ],
        ];

        $this->mockRepository->expects($this->once())
            ->method('countPlaysForContributorIds')
            ->with([1, 2], $from, $to, null)
            ->willReturn($dbData);

        $results = $this->service()->countPlaysForTimeByPid(
            [$contributor1, $contributor2], // order shouldn't matter
            $from,
            $to
        );
        $this->assertSame(2, count($results));
        $this->assertSame(10, $results[$pid1]);
        $this->assertSame(5, $results[$pid2]);
    }

    public function testAllEmpty()
    {
        $from = new \DateTimeImmutable('2016-06-01');
        $to = new \DateTimeImmutable('2016-06-07');

        $mbid1 = '7746d775-9550-4360-b8d5-c37bd448ce01';

        $pid1 = 'cntrbtr1';

        $contributor1 = new Contributor(
            1,
            new Pid($pid1),
            'person',
            'Number 1',
            $mbid1
        );

        $dbData = [];

        $this->mockRepository->expects($this->once())
            ->method('countPlaysForContributorIds')
            ->with([1], $from, $to, null)
            ->willReturn($dbData);

        $results = $this->service()->countPlaysForTimeByPid(
            [$contributor1],
            $from,
            $to
        );
        $this->assertEquals([], $results);
    }

    public function testByService()
    {
        $from = new \DateTimeImmutable('2016-06-01');
        $to = new \DateTimeImmutable('2016-06-07');
        $service = new Service(1, new Sid('sid'), 'name');

        $mbid1 = '7746d775-9550-4360-b8d5-c37bd448ce01';

        $pid1 = 'cntrbtr1';

        $contributor1 = new Contributor(
            1,
            new Pid($pid1),
            'person',
            'Number 1',
            $mbid1
        );

        $dbData = [
            [
                0 => [
                    'pid' => $pid1,
                    'musicBrainzId' => $mbid1,
                ],
                'contributionPlays' => 10,
            ],
        ];

        $this->mockRepository->expects($this->once())
            ->method('countPlaysForContributorIds')
            ->with([1], $from, $to, 1)
            ->willReturn($dbData);

        $results = $this->service()->countPlaysForTimeByPid(
            [$contributor1], // order shouldn't matter
            $from,
            $to,
            $service
        );
        $this->assertSame(1, count($results));
        $this->assertSame(10, $results[$pid1]);





        $from = new \DateTimeImmutable('2016-06-01');
        $to = new \DateTimeImmutable('2016-06-07');

        $mbid = '6746d775-9550-4360-b8d5-c37bd448ce01';
        $dbData = [[
            0 => ['musicBrainzId' => $mbid],
            'contributionPlays' => 10,
        ]];

        $this->mockRepository->expects($this->once())
            ->method('findAllMostPlayedWithPlays')
            ->with($from, $to, 1)
            ->willReturn($dbData);

        $results = $this->service()->findAllMostPlayed($from, $to, $service);
        $this->assertSame(1, count($results));
        $this->assertEquals($this->contributorFromDbData($dbData[0][0]), $results[0]->contributor);
        $this->assertSame(10, $results[0]->plays);
    }
}
